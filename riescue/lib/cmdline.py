...
class CmdLine:
    ...
    def __init__(self, cmdline_json, args_to_process=None, parser_extensions=None):
        """
        ...
        """
        self.parser = JsonArgumentParser.from_json(cmdline_json)
        ...
        if parser_extensions is not None:
            ...
            for obj in parser_extensions:
                if not hasattr(obj, "add_args"):
                    raise AttributeError(f"parser_extensions objects {obj} does not have an 'add_args' method.")
                if not callable(obj.add_args):
                    raise TypeError(f"parser_extensions objects {obj} 'add_args' method is not callable.")
                obj.add_args(self.parser)
