...
def setup_logging(verbose):
    ...
    """
    if verbose:
        logging.basicConfig(format="%(asctime)s %(filename)s:%(lineno)-5s %(levelname)-8s %(message)s", datefmt="%a, %d %b %Y %H:%M:%S", level=logging.DEBUG)
    else:
...
def run_cmd(cmd, timeout_s=999, exit_on_error=1, check_return_code=True, debug_cmd=None):
    ...
    if debug_cmd:
        ...
        return
    try:
        ps = subprocess.Popen("exec " + cmd, shell=True, executable="/bin/bash", universal_newlines=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError:
        logging.error(ps.communicate()[0])  # type: ignore
        sys.exit(RET_FAIL)
    ...
    if rc and check_return_code and rc > 0:
        logging.info(output)
        logging.error("ERROR return code: {}/{}, cmd:{}".format(check_return_code, rc, cmd))
        if exit_on_error:
            sys.exit(RET_FAIL)
...
def run_parallel_cmd(cmd_list, timeout_s=999, exit_on_error=0, check_return_code=True, debug_cmd=None):
...
def process_regression_list(testlist, test, iterations, matched_list, riscv_dv_root):
    ...
    """
    logging.info("Processing regression test list : {}, test: {}".format(testlist, test))
        ...
        if "import" in entry:
            ...
            if (entry["test"] in mult_test) or (test == "all"):
                ...
                if entry["iterations"] > 0:
                    logging.info("Found matched tests: {}, iterations:{}".format(entry["test"], entry["iterations"]))
