...
from typing import Tuple, Iterator, Sequence, Any, Iterable
...
class _IntervalAddressRangeSet(AddressRangeSet):
    ...
    def __init__(self, ranges: Iterable[AddressRange] = ()):
        self._container = IntervalTree(Interval(start, end + 1) for start, end in ranges)
...
def address_range_set(ranges: Iterable[AddressRange] = ()) -> AddressRangeSet:
    """
    Factory function to create a new AddressRangeSet.

    :return: AddressRangeSet
    """
    return _IntervalAddressRangeSet(ranges)
