...
# Showcase changing the paging modes defined with ;page_map() syntax
.ifne PRIV_MODE_SUPER && !PAGING_MODE_DISABLE
# .ifne 0
    # Update SATP with map1_sptbr
    # Read value of SATP into x1
    csrr x1, satp
    # Update PPN from t3
    ...
    # Do a ld with add lin2
    li x3, lin2
    ld t3, 0(x3)

    # Now do the same thing for the map2
    # Update SATP with map2_sptbr
    # Read value of SATP into x1
    csrr x1, satp
    # Update PPN from t3
    li t3, 0x0ffff00000000000
    and x1, x1, t3
    li t3, (map2_sptbr>>12) | 0x9000000000000000
    or x1, x1, t3
    # Update SATP
    csrw satp, x1
    sfence.vma
    ...
    # Now do the same thing for the map3
    # Update SATP with map3_sptbr
    # Read value of SATP into x1
    csrr x1, satp
    sfence.vma
    # Update PPN from t3
    li t3, 0x0ffff00000000000
    and x1, x1, t3
    li t3, (map3_sptbr>>12) | 0xa000000000000000
    or x1, x1, t3
    # Update SATP
    csrw satp, x1
    sfence.vma
    ...
    # Change SATP back to map_os, x1 has the current value of satp
    li t3, 0x0ffff00000000000
    and x1, x1, t3
    .ifne PAGING_MODE_DISABLE
        ...
        li t3, 0
    .endif
    .ifne PAGING_MODE_SV39
        li t3, (map_os_sptbr>>12) | 0x8000000000000000
    .endif
    .ifne PAGING_MODE_SV48
        li t3, (map_os_sptbr>>12) | 0x9000000000000000
    .endif
    .ifne PAGING_MODE_SV57
        li t3, (map_os_sptbr>>12) | 0xa000000000000000
    .endif
    or x1, x1, t3
    # Update SATP
    csrw satp, x1
...
# Sample test for access fault and PMA support testing
.ifne PMA_ENABLED
...
#####################
# Another user defined data section located at address lin3 that has code
#####################
;#init_memory @lin3
    .dword 0xc001c0de

#####################
# Another user defined data section located at address lin4 that has code
#####################
;#init_memory @lin4
    .dword 0xc001c0de

#####################
# Another user defined data section located at address lin5 that has code
#####################
;#init_memory @lin5
    jr x3   # jump to
    .dword 0xc001c0de
...
#####################
# Another user defined data section located at address lin6 that has code
#####################
;#init_memory @lin6
    .dword 0xc001c0de
.org 0x1000
...
#####################
# Another user defined data section located at address lin7 that has code
#####################
;#init_memory @lin7
    .dword 0xc001c0de
.org 0x1000
...
#####################
# Another user defined data section located at address 0x600000000 that has code
#####################
...
#####################
# Another user defined data section located at address 0x500000000 that has code
#####################
...
#####################
# Another user defined data section located at address my_code_page that has code
#####################
