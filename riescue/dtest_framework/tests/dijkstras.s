...
;#page_mapping(lin_addr=0x6000000, phys_name=&random, v=1, r=1, w=1, x=1, a=1, d=1, pagesize=['4kb'])
...
test02:
    ...
    # Define the adjacency matrix and costs array
    #  0  1  2  3  4
    # ----------------
    #0| 0  3  0  4  0
    #1| 3  0  1  5  0
    ...
    # la a0, _adjacency_matrix
    # la a1, _costs_array
    li a0, data
    li a1, lin1
    li a2, 0    # initialize Dijkstra's starting node to 0
    call dijkstra
...
# Dijkstra's algorithm on adjacency matrix with costs
dijkstra:
    # Set up visited and costs arrays
    addi sp, sp, -24
    sd ra, 0(sp)
    sd s0, 8(sp)
    sd s1, 16(sp)

    li t2, 5   # number of nodes in the graph
    li t3, 0   # initialize visited array to all 0s
    li t4, -1  # initialize costs array to all -1s
    mv t5, a2 # starting node
    li t6, 0   # initialize cost of starting node to 0
    mv a3, a0
    add a3, a3, t5
    sb t3, 0(a3)
    mv a3, a1
...
dijkstra_loop_check:
    bge s0, t2, dijkstra_exit
    li s1, 0 # minimum cost
    li s2, 0 # minimum cost node
    li s3, 0 # neighbor index
dijkstra_find_min_cost_node_check:
    blt s3, t2, dijkstra_find_min_cost_node_body
    beq s2, x0, dijkstra_update_costs_check
    mv a3, s2
    add a3, a3, a0
    sb t3, 0(a3) # mark node as visited
    add s0, s0, 1 # increment loop counter
    j dijkstra_loop_check

dijkstra_find_min_cost_node_body:
    addi s3, s3, 1
    mv a3, s3
    add a3, a3, a0
    lbu s5, 0(a3)
    beq s5, x0, dijkstra_find_min_cost_node_check
    mv a3, a1
    add a3, a3, s1
    add a3, a3, s2
    lw s6, 0(a3)
    ...
    add a3, a3, s3
    lw s8, 0(a3)
    blt s8, x0, dijkstra_update_costs
    blt s7, s8, dijkstra_update_costs
    mv s1, s7
    mv s2, s3
    j dijkstra_find_min_cost_node_check

dijkstra_update_costs_check:
    addi s3, s3, 1
dijkstra_update_costs_body:
    blt s3, t2, dijkstra_update_costs_body
    beq s2, x0, dijkstra_find_min_cost_node_check
    mv a3, a1
    add a3, a3, s1
    add a3, a3, s2
    sw s1, 0(a3) # update cost of minimum cost node
    mv a3, a0
    add a3, a3, s2
    sb t3, 0(a3) # mark node as visited
    mv s4, s2
    li s2, -1 # reset minimum cost node
    li s1, -1 # reset minimum cost
    li s3, 0 # reset neighbor index
    dijkstra_update_costs:
    mv a3, a0
    add a3, a3, s3
    lbu s9, 0(a3)
    beq s9, x0, dijkstra_update_costs_check
    mv a3, a1
    add a3, a3, s1
    add a3, a3, s4
    lw s10, 0(a3)
    ...
    add a3, a3, s3
    lw a2, 0(a3)
    blt a2, x0, dijkstra_update_costs_check
    blt s11, a2, dijkstra_update_costs_check
    mv a3, a1
    add a3, a3, s1
    add a3, a3, s3
    sw s11, 0(a1)
    mv a3, a0
    add a3, a3, s4
    sb s3, 0(a3) # record previous node for shortest path
    j dijkstra_update_costs_check
