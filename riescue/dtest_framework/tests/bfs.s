...
.equ num_of_nodes, 50
    ...
    li a0, data
    li a1, 0    # initialize BFS starting node to 0
    call bfs
...
# BFS algorithm on adjacency matrix
bfs:
    # Set up queue and visited array
    addi sp, sp, -24
    sd ra, 0(sp)
    sd s0, 8(sp)
    sd s1, 16(sp)

    li t0, 0   # initialize queue front
    li t1, 0   # initialize queue back
    li t2, num_of_nodes   # number of nodes in the graph
    li t3, 0   # initialize visited array to all 0s
    mv t4, a1 # starting node
    mv a2, t4
    add a2, a2, a0
    sb t3, 0(a2)  # mark starting node as visited
    addi t1, t1, 1
    mv t5, t4     # enqueue starting node
    mv a2, t1
    add a2, a2, a0
    sw t5, 0(a2)
...
bfs_loop:
    ...
    # Visit neighbors of the dequeued node
    li t6, 0 # neighbor index
    j bfs_check_neighbors

bfs_visit_neighbor:
    # Check if neighbor has been visited
    addi t6, t6, 1
bfs_check_neighbors:
    blt t6, t2, bfs_visit_neighbor
    j end_bfs_loop
    ...
    add a2, a2, a0
    lbu a3, 0(a2)
    bnez a3, bfs_check_neighbors # skip if neighbor already visited
    mv a2, t4
    add a2, a2, a0
    sb t3, 0(a2) # mark neighbor as visited
    addi t1, t1, 1
    mv a2, t1
    add a2, a2, a0
    sw t4, 0(a2)
...
end_bfs_loop:
    # Clean up queue and visited array
    ld ra, 0(sp)
    ld s0, 8(sp)
    ld s1, 16(sp)
    addi sp, sp, 24
