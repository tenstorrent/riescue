...
It runs in Machine mode (M-mode).
...
While in the ``Runtime Environment``, the hart has access to a thread-local storage  or ``Hart Context``.
This is used to store the test's context during a trap, to ensure that asynchronou or unexpected traps don't corrupt the test's state.

The ``Hart Context`` is maintained until the ``Runtime`` returns control back to the ``Test``

See the :doc:`Variables <variables>` for info on using the Hart Context.
...
It always runs in Machine mode (M-mode).
...
Exception delegation registers (``medeleg``/``mideleg``/``hedeleg``/``hideleg``)
control how non-ecall traps get delegated. By default, these are set randomly.
