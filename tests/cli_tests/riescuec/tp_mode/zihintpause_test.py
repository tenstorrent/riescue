...
class ZihintpauseTest(BaseRiescueCTest):
    "Runs Zihintpause (Pause Hint) test plan"

    def test_cli(self):
        self.run_tp_mode(plan="zihintpause", cli_args=["--conf", str(CUSTOM_MAPPING_CONF), "--whisper_config_json", str(WHISPER_CONFIG_JSON)])
