...
class HypervisorPagingCsrAdTest(BaseRiescueCTest):
    "Runs hypervisor_paging_csr_ad test plan"

    def test_cli_machine_mode(self):
        self.run_tp_mode(plan="hypervisor_paging_csr_ad", cli_args=["--test_priv_mode", "machine", "--repeat_times", "1"])

    def test_cli_bare_metal_super(self):
        self.run_tp_mode(plan="hypervisor_paging_csr_ad", cli_args=["--test_priv_mode", "super", "--test_env", "bare_metal", "--repeat_times", "1"])
