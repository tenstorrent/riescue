...
class HypervisorPagingFaultsVsTest(BaseRiescueCTest):
    "Runs hypervisor_paging_faults_vs test plan (VS-stage fault scenarios)"

    def test_cli_gstage_only(self):
        for g_paging_mode in ("sv39", "sv48", "sv57"):
            for priv_mode in ("super", "user"):
                self.run_tp_mode(
                    plan="hypervisor_paging_faults_vs",
