...
class HypervisorPagingFaultsGReservedTest(BaseRiescueCTest):
    "Runs hypervisor_paging_faults_g_reserved test plan (G-stage reserved bit fault scenarios)"

    def test_cli_two_stage(self):
        """All scenarios in this test plan require VS-stage paging enabled (SV39/SV48/SV57)."""
        for paging_mode in ("sv39", "sv48", "sv57"):
            for g_paging_mode in ("sv39", "sv48", "sv57"):
                for priv_mode in ("super", "user"):
                    self.run_tp_mode(
                        plan="hypervisor_paging_faults_g_reserved",
